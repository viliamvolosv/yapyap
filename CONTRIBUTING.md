...
## Before you start
1. Run `npm install` to ensure your lockfile is current.
...
## Local workflow
- Build the project with `npm run build` and the CLI with `npm run build:cli`. The `yapyap` binary lives in `dist/` after building.
- Run `npm test` (or targeted `.test.ts` files) to validate behavior.
- Enforce linting and formatting by running `npm run lint`, `npm run format`, `npm run check`, and `npm run typecheck` before submitting.
